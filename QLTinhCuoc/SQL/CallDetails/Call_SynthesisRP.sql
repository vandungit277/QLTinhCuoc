...
Author: Dungdv24
Desc: 11/11/2018
Desc: Báo cáo cước gọi tổng hợp
*/
ALTER PROCEDURE Call_SynthesisRP
...
AS
    BEGIN
        ...
        DECLARE @query NVARCHAR(MAX) = N'SELECT S.UserName ,
         SUM(CD.TotalTimeCalled)  AS TotalTimeCalled ,
     ...
     ' + @qr +N' GROUP BY UserName,B.BranchName,D.DepartmentName, C.CompanyName '
        PRINT @query
