$(document).ready(function () {
    ...
    $("#idbtnsearch").click(function () {
        ...
        var groupsId = $("#idgroup").data("kendoDropDownList").dataItem().GroupsID;
       var data = {
           "Search": search, "GroupsID": groupsId, "MenuRootID": menuroot
        };
        CallListMenuGroup(data);
    });
...
function CallListMenuGroup(data) {
    window.$.ajax({
        type: "POST",
        async: false,
        url: "/CallMenu/Menu/Call_ListGetMenuGroup",
        processData: false,
        cache: false,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function OnOfMenu(element) {
    ...
    //    }
    //});
    var data = { "GroupsID": row.GroupsID, "MenuID": row.MenuID, "Status": st, "UpdateBy": "System" }
    kendo.confirm("Bạn có muốn " + vNrow + " Menu này?").then(function () {
        window.$.ajax({
            url: "/CallMenu/Menu/Call_IUGroupsMenu",
            type: "Post",
            dataType: "json",
            data: data,
            async: true,
